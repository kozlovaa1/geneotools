...
function createSyntheticDatabase(SQL, roleIds) {
  ...
  execStatements(db, [
    ...
    'CREATE TABLE ValuesDates (id INTEGER PRIMARY KEY, f_id INTEGER NOT NULL, rec_table INTEGER NOT NULL, rec_id INTEGER NOT NULL, y INTEGER, m INTEGER, d INTEGER)',
  ...
  insertRows(db, 'INSERT INTO Places (id) VALUES (?)', [[100], [200]]);
  ...
  insertRows(db, 'INSERT INTO ValuesDates (id, f_id, rec_table, rec_id, y, m, d) VALUES (?, ?, ?, ?, ?, ?, ?)', [
    [1, fields.eventDate.id, tableCodes.events.code, 1000, 1901, 2, 3],
    [2, fields.eventDate.id, tableCodes.events.code, 2000, 1950, 0, 0],
  ]);
...
function assertParsedContract(parsed, diagnostics) {
  ...
  assert.equal(birthEvent.place, 'SyntheticBirthPlace', 'event birth place mismatch');
  assert.equal(deathEvent.date, '1950-00-00', 'event death date mismatch');
  assert.equal(deathEvent.place, 'SyntheticDeathPlace', 'event death place mismatch');

  for (const fieldName of ['eventDate', 'eventPlaceLink', 'placeName']) {
    assert.ok(
      diagnostics.some(
        (diagnostic) =>
