...
Машиночитаемый канон находится в `lib/atdb/mapping.json`. TypeScript runtime загружает его через `lib/atdb/mapping.ts`, а Node.js-проверки читают тот же JSON напрямую. Числовые коды не должны независимо дублироваться в readers, writers или проверочных скриптах.

Подтверждённый общий baseline трёх разрешённых fixtures:

| Сущность | `rec_table` | Уровень | Чтение | Запись |
|----------|-------------|---------|--------|--------|
| `Events` | `7` | `invariant` | да | да |
| `Families` | `9` | `invariant` | да | да |
| `Persons` | `13` | `invariant` | да | да |
| `Places` | `14` | `invariant` | да | да |

Уровни правил:

- `invariant` — подтверждено всеми разрешёнными fixtures или повторяемым controlled diff; только такие правила могут быть `write-safe`;
- `fixture-specific` — наблюдение подтверждено не для всех fixtures; доступно для терпимого чтения, но не для записи;
- `legacy-fallback` — совместимый fallback для чтения старых вариантов; применение должно создавать безопасную диагностику `WARN`;
- неизвестные коды сохраняются как данные, но не получают универсальную семантику.

`AtdbSchemaContext` разрешается один раз на открытую базу. Он читает каталоги `Fields`, `EventTypes`, `EventRoles`, хранит выбранные правила и передаётся readers/writers. `Fields.area` не считается самостоятельным доказательством семантики.

### Контракт безопасной записи
...
Runtime по умолчанию использует беззвучный logger. Внедряемые диагностические события содержат только уровень, код правила и безопасные счётчики:

- `DEBUG` — выбранные правила и количество структур;
- `WARN` — legacy fallback, неоднозначность или пропуск небезопасной записи;
- `ERROR` — нарушение обязательного инварианта.

Обязательные проверки mapping:
...
Содержит базовые записи родов. В русскоязычном UI «Древо Жизни 6» эта сущность соответствует разделу «Роды», а не нуклеарным семьям.

Род — это именованная группа персон, обычно связанных общей фамилией. К одному роду может быть привязано неограниченное количество персон. У записи рода нет доменных ролей «муж», «жена» и «дети»; такие отношения нужно извлекать из событий и ролей событий, а не из `Families`.
...
Человекочитаемые названия родов хранятся в `ValuesStr` при `rec_table = 9`.
...
- `48` -> мужская форма фамилии рода
- `49` -> женская форма фамилии рода
- `50` -> отображаемое имя рода / фамилия во множественном числе
- `52` -> комментарий
