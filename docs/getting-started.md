...
3. После парсинга будут доступны вкладки:
   ...
   - События
   - Места
4. Используйте быстрый поиск текущей вкладки или один field-level фильтр:
   - `contains` и `equals` используют введённое значение;
   - `empty` и `not empty` работают без значения;
   - фильтр применяется только к активной вкладке;
   - счётчик показывает, сколько строк видно из общего количества строк вкладки.
5. Сортируйте таблицу кнопками в заголовках колонок. Сортировка хранится отдельно для каждой вкладки и применяется к уже отфильтрованной выборке.
6. Отредактируйте разрешённые поля прямо в таблицах:
...
7. Для массового редактирования выберите строки на вкладке персон, родов или мест, откройте «Массовое редактирование», выберите поле, область и операцию, затем сначала построите предпросмотр.
8. Checkbox «выбрать все» в таблице выбирает только текущие видимые строки после поиска/фильтра. Уже выбранные, но скрытые фильтром строки остаются выбранными до явного сброса.
9. Доступные области массового редактирования:
   - выбранные строки;
...
11. Предпросмотр показывает affected/skipped/no-op counts, reason codes и изменения по ID. Кнопка применения активна только для актуального предпросмотра.
12. Применение массового редактирования меняет только локальный draft. Скачивание `.atdb` остаётся отдельным шагом и использует общий `AtdbChangeSet`.
13. Вкладка событий остаётся только для просмотра: тип события, дата, место, описание и участники не записываются.
14. Счётчик рядом с кнопкой скачивания показывает количество изменённых полей и записей. Пока изменений нет, кнопка скачивания обновлённого файла отключена и сборка не запускается.
15. Скачайте обновлённый `.atdb`. Изменения применяются только к скачанному файлу; исходный локальный файл не перезаписывается.
...
- Быстрый поиск, field-level фильтр, очистка фильтра и сортировка работают на текущей вкладке
- Checkbox «выбрать все» выбирает текущие видимые строки, а скрытые выбранные строки не теряются
...
- Глобальный поиск по всем вкладкам, сложный конструктор нескольких условий и виртуализация больших таблиц остаются отдельными этапами
