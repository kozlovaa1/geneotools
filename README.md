...
Требуется Node.js 22.x и npm 10.x. В проекте есть `.nvmrc`, поэтому при использовании `nvm` сначала выполните `nvm use`.
...
- **Write-safe редактирование** — изменяются только разрешённые поля персон, родов, событий и мест.
- **Массовое редактирование** — выбранные строки, видимая выборка или строки по простому условию проходят через предпросмотр перед применением; inline-only поля редактируются прямо в таблице.
...
- Разрешены только update-only изменения существующих записей. Для событий доступна только привязка места, а даты рождения/смерти записываются через primary life-event при простой структуре `ValuesDates`.
- Остальные атрибуты событий, участники событий, родственные связи, metadata, `Global`, `Fields`, `Recs`, `EventRoles`, custom fields и неоднозначные `Values*` остаются только для чтения.
